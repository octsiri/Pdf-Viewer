    ...
    fun downloadFile(context: Context, assetName: String, filePath: String, fileName: String?){
        try {
            ...
            if (intent.resolveActivityInfo(context.packageManager, 0) != null)
            {
                ...
                val toast = Toast.makeText(context, "Successfully Save PDF To $assetName folder", Toast.LENGTH_LONG)
                toast.show()
            }
            else
            {
                ...
                val toast = Toast.makeText(context, "Failed Save PDF To $assetName folder", Toast.LENGTH_LONG)
                toast.show()
            }
        } catch(e: Exception) {
            val sw = StringWriter()
